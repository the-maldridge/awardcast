...
// newES returns an initialized event streamer ready for use.
func newES() *EventStream {
	es := &EventStream{
		maxUndelivered: 16,
...
func (es *EventStream) Handler(w http.ResponseWriter, r *http.Request) {
	slog.Debug("Incomming subscriber", "remote-addr", r.RemoteAddr)
	...
	if err != nil {
		slog.Warn("Error handling subscription request", "error", err)
		return
	}
}
